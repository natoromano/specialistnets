...
dustbin scores. Need to run specialist_scores before this.
This code is inspired by Sergey Zagoruyko, cf 
https://github.com/szagoruyko/cifar.torch ]]--

--[[ To predict:
compute the probability distribution for all specialists and renormalize it for
100 classes
https://github.com/szagoruyko/cifar.torch ]]--
...
function create_prob_distribution(inputs, domains)
  ...
  probability distribution ]]--
    for i, domain in pairs(domains) do
      local dim = #domain + 1
      -- mul by #domain/100 to take into account the size of the cluster
      probs = sm:forward(inputs[{{},{id+1, id+dim}}]):mul(#domain/100)
      for k, class in pairs(domain) do
        outputs[{{},class}] = probs[{{},{k}}]
      end
      id = id + dim
    end
  return outputs
